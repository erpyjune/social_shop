...
class First {
	...
	var $body_s = '<ul class="list" id="bestList_0" name="bestList"  style="display:none;"  >';
	var $body_e = '<div class="list_count">';
	...
	public function parsePrdtInfo($result_list, $cp, $pa) {
		...
		foreach ($result_list as $list) {
			...
			foreach ($nodelist as $n){
				$s = utf8_decode($n->nodeValue);
				$t = substr($s, 0, -5);
				$t_thumb = "http://www.chocammall.co.kr" . $t. "0.jpg";
			}
			...
			foreach ($nodelist as $n){
				$s = utf8_decode($n->nodeValue);
				$t = $pa->getItem($s, '(', ')');
				$t_link = "http://www.chocammall.co.kr/shop/base/product/viewProductDetail.do?goods_no=" . $t;
			}
		...
		for ($i=0; $i<$total; $i++) {
			...
			if ($db->data_exist($s_sql) == 0) { // db에 link가 없다면 insert.
				...
				$t_sql = "INSERT INTO SOCIAL_SHOP_T (title, cmt, brand, link, thumb, price_org, price_sale, price_special, sale_per, sell_count, cate, crawl_url, in_timestamp, cp)
					VALUES ('$t_title', '$cmt', '$t_brand', '$t_link', '$t_thumb', '$t_price_org', '$t_price_sale', '$t_price_special', '$t_sale_per', $t_sell_count, '$t_cate', '$crawl_url', $reg_datetime, 'first')";
				$db->select($t_sql);
				echo "(INSERT) $t_title\n";
